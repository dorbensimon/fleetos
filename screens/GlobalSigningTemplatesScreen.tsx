...
import { CONTENT_MAX_WIDTH, FONT_SIZE, BRAND as BRAND_TOKENS } from '../lib/theme';

type Props = NativeStackScreenProps<RootStackParamList, 'GlobalSigningTemplates'>;
type Tab = 'ready' | 'archive';
const BRAND = '#0088CC'; const TEXT = BRAND_TOKENS.ink; const MUTED = BRAND_TOKENS.inkSecondary;
...
export default function GlobalSigningTemplatesScreen({ navigation }: Props) {
  ...
  </ScrollView><Modal visible={!!renaming} transparent animationType="fade" onRequestClose={() => !savingName && setRenaming(null)}>
...
function TemplateRow({ item, last, loading, onOpen, onDownload, onArchive, onRename }: { item: SigningTemplate; last: boolean; loading: boolean; onOpen: () => void; onDownload: () => void; onArchive: () => void; onRename: () => void }) { return <TouchableOpacity activeOpacity={.75} disabled={loading} onPress={onOpen} onLongPress={onDownload} style={[s.listRow, !last && s.listDivider]}><View style={s.documentIcon}><Ionicons name={loading ? 'hourglass-outline' : 'document-text-outline'} size={18} color="#FFF" /></View><View style={s.rowCopy}><AppText numberOfLines={1} style={s.rowTitle}>{item.title}</AppText><AppText numberOfLines={1} style={s.rowSubtitle}>משותפת לכל החברות</AppText></View><TouchableOpacity hitSlop={6} style={s.smallAction} onPress={onDownload}><Ionicons name="download-outline" size={16} color={MUTED} /></TouchableOpacity><TouchableOpacity accessibilityLabel="שינוי שם תבנית" hitSlop={6} style={s.smallAction} onPress={onRename}><Ionicons name="pencil-outline" size={16} color={BRAND} /></TouchableOpacity><TouchableOpacity hitSlop={6} style={s.deleteAction} onPress={onArchive}><Ionicons name="trash-outline" size={16} color="#C0392B" /></TouchableOpacity></TouchableOpacity>; }
...
const s = StyleSheet.create({
  ...
  error: { marginHorizontal: 20, marginTop: 12, textAlign: 'center', fontFamily: 'Assistant_600SemiBold', color: '#C0392B' }, section: { paddingHorizontal: 20, paddingTop: 20 }, caption: { fontFamily: 'Assistant_600SemiBold', fontSize: FONT_SIZE.md, color: BRAND_TOKENS.inkSecondary },

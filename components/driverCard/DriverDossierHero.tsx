...
export function DriverDossierHero({ title, subtitle, icon, onBack, insetTop = 0 }: {
  ...
  return <View style={[styles.wrap, { paddingTop: Math.max(insetTop, 12) + 8 }]}>
    <View style={styles.topRow}>
      ...
      <LinearGradient colors={['#3FA9E8', '#0A7FD0']} style={styles.icon}><Ionicons name={icon} size={42} color="#FFFFFF" /></LinearGradient>
    </View>
...
const styles = StyleSheet.create({
  ...
  icon: { position: 'absolute', alignSelf: 'center', width: 72, height: 72, borderRadius: 36, alignItems: 'center', justifyContent: 'center', shadowColor: '#0A7FD0', shadowOpacity: 0.28, shadowRadius: 13, shadowOffset: { width: 0, height: 8 }, elevation: 6 },
  title: { marginTop: 7, fontSize: 25, letterSpacing: -0.5, color: '#0E1E2B', textAlign: 'center' },
